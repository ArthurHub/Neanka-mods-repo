...
namespace DialogueMenuPlacement
{
    namespace
    {
        ...
        /**
         * The handle of the NPC the dialogue is with, 0 when there is no dialogue. Taken from the topic
         * manager, which holds the speaker for the whole conversation, rather than from the player dialogue
         * action, which only answers while one is running - hardly ever, at the rate this is asked.
         */
        UInt32 GetTargetHandle()
        {
            const UInt32 handle = DialogueEx::GetDialogueTargetHandle();
            if (handle && handle != s_targetHandle) {
                s_targetHandle = handle;
                // A new NPC means a new conversation as far as this is concerned: pick up an MCM change, and
                // start the aim on the NPC rather than sweeping onto them from wherever the last one was.
                s_enabled = Settings::GetBool("bEnableDialogueUIFacingNpc:VR", true);
        ...
        /**
         * Lets the anchor yaw sit as far from the head as the NPC needs it to, for as long as there is an NPC.
         * Putting the setting back when the conversation ends keeps the engine's own behaviour for every other
         * menu. Only the parsed setting in memory is touched, never the ini on disk.
         */
        void RaiseLagClamp(const bool raised)
        {
            if (raised == s_lagMaxDistanceRaised) {
                return;
            }
        ...
        void AimUIAtDialogueTarget(void* player, float deltaSeconds)
        {
            ...
            const UInt32 handle = GetTargetHandle();
            if (!handle || !s_enabled) {
                RaiseLagClamp(false);
                return;
            }

            const BorrowedRef target(handle);
            if (!target.Get()) {
                RaiseLagClamp(false);
                return;
            }
