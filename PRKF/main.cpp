...
public:
	virtual void	Invoke(Args * args) {
		if (args->args[0].GetType() != GFxValue::kType_String) return;
		auto soundDescriptorForm = BGSAudio__GetSoundDescriptor(args->args[0].GetString());
		if (!soundDescriptorForm || soundDescriptorForm->formType == kFormType_SNDR) return;
		LevelUpMenu* menu = DYNAMIC_CAST((*g_ui)->GetMenu(BSFixedString("LevelUpMenu")), IMenu, LevelUpMenu);
		if (!menu) return;
		BSSoundHandle__Stop(&menu->playingSoundHandle);
		BSAudioManager__GetSoundHandle(*BSAudioManager_instance, &menu->playingSoundHandle, &((BGSSoundDescriptorForm*)soundDescriptorForm)->soundDescriptor, 0.0, 0x10, 0);
		BSSoundHandle__Play(&menu->playingSoundHandle);
	}
};
...
{
	_MESSAGE("PRKF_Papyrus::OpenTagSkillsMenu %i %i",numSkills, numPoints);
...
extern "C"
{
	...
	{
		gLog.OpenRelative(CSIDL_MYDOCUMENTS, (const char*)("\\My Games\\Fallout4VR\\F4SE\\" + mName + ".log").c_str());
